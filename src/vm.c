...
#define BINARY_OP(valueType, op)                                               \
  ...
  for (;;) {
    ...
    case OP_SET_GLOBAL: {
      ObjString *name = READ_STRING();
      // tableSet returns true if 'name' is a new key
    ...
    case OP_CLOSURE: {
      ...
      for (int i = 0; i < closure->upvalueCount; ++i) {
        ...
        if (isLocal) {
          ...
          // When OP_CLOSURE executes the current current function is the enclosing 
          // function of the closure, so it is currently stored in the frame variable.
          // There we can grab the non local upvalue easily.
